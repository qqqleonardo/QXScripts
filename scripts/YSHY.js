...
配置 (QuanX)
[MITM]
hostname = hy.51pt.top

[rewrite_local]
^https:\/\/hy\.51pt\.top\/app\/ys\/appletTask\/getTask\?isdefault\=1 url script-request-header https://raw.githubusercontent.com/qqqleonardo/scripts/refs/heads/main/YSHY.js


[task_local]
25 6 * * * https://raw.githubusercontent.com/qqqleonardo/scripts/refs/heads/main/YSHY.js, tag=仰韶会员俱乐部小程序, enabled=true
====================================================================================================
*/
const $ = new Env("仰韶会员俱乐部小程序");
const GLJJ =
  ($.isNode() ? JSON.parse(process.env.YSHY) : $.getdata("YSHY")) || [];
let notice = "";
!(async () => {
...
async function main() {
  ...
  await $.wait(3000);
  console.log("\n——————");
  let info = await commonGet("/convert/getMemberScore", token);
